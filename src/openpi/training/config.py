                ...
                _transforms.RepackTransform(
                    {
                        ...
                        "observation/wrist_image": "observation.images.hand_camera",
                        # "observation/base_camera": "image",
...
@dataclasses.dataclass(frozen=True)
class LeRobotNoahBiArmDataConfig(DataConfigFactory):
    """
    This config is used to configure transforms that are applied at various parts of the data pipeline.
    For your own dataset, you can copy this class and modify the transforms to match your dataset based on the
    comments below.
    ...
    #@sajjad: have to set place holder values as the class was inherited from its parent
    repo_id: str = "robot-uid_control-mode/env-id"
    env_id: str = "env-id"
    ...
    def create(
        ...
        # Below, we define the transforms for data going into the model (``inputs``) and the transforms
        # for data coming out of the model (``outputs``) (the latter is only used during inference).
        # We defined these transforms in `noahbiarm_policy.py`. You can check the detailed comments there for
        # how to modify the transforms to match your dataset. Once you created your own transforms, you can
        # replace the transforms below with your own.
        data_transforms = _transforms.Group(
            inputs=[
                noahbiarm_policy.NoahBiArmInputs(
                    action_dim=model_config.action_dim,
                    model_type=model_config.model_type,
                )
            ],
            outputs=[noahbiarm_policy.NoahBiArmOutputs(
                    robot_action_dim=self.get_robot_action_dim()
                )
            ],
        )
        ...
        if self.should_apply_delta_trans():
            delta_action_mask = _transforms.make_bool_mask(self.get_robot_action_dim()-1, -1)
            data_transforms = data_transforms.push(
                inputs=[_transforms.DeltaActions(delta_action_mask)],
                outputs=[_transforms.AbsoluteActions(delta_action_mask)],
            )
...
_CONFIGS = [
    ...
    TrainConfig(
        ...
        data=LeRobotManiskillDataConfig(
            repo_id="aaronwool2025/noah_fork",
            base_config=DataConfig(
    ...
    TrainConfig(
        ...
        # Also modify the DataConfig to use the new config you made for your dataset above.
        data=LeRobotManiskillDataConfig(
            repo_id="aaronwool2025/plateonrack_wristcam",
            base_config=DataConfig(
                local_files_only=True,  # Set to True for local-only datasets.
                # This flag determines whether we load the prompt (i.e. the task instruction) from the
                # ``task`` field in the LeRobot dataset. If set to True, the prompt will show up in
    ...
    TrainConfig(
        ...
        data=LeRobotNoahBiArmDataConfig(
            repo_id = "noahbiarm_rc_pd_joint_pos/PlaceKnifeOnRack-v1-RL4",
            env_id = "PlaceKnifeOnRack-v1-RL4",
